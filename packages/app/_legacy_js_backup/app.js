...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // startBatchScan moved to TypeScript module (batch-scan-legacy.ts). Keep a thin delegator for any
  // lingering references or tests that still call window.startBatchScan directly.
  async function startBatchScan() {
    if (window.LegacyBatchScan && typeof window.LegacyBatchScan.start === 'function') {
      return window.LegacyBatchScan.start();
    }
    console.warn('[app] LegacyBatchScan module not loaded yet');
  }
  ...
  // Batch scan button listener now owned by batch-scan-legacy.ts; avoid double-binding here.
